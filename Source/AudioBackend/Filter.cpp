...
Filter::Filter(std::vector<float> ai, std::vector<float> bi) {
  ...
  if (ai.size() != 4) {
    throw std::runtime_error(
        "Only filters of order 4 are supported by this class.");
  }
...
float Filter::filter(float sample) {
  ...
  case 0:
    iter = 1;
    x[0] = sample;
    y[0] = (b[0] * x[0]) + (b[1] * x[3]) + (b[2] * x[2]) + (b[3] * x[1]) -
  ...
  case 1:
    iter = 2;
    x[1] = sample;
    y[1] = (b[0] * x[1]) + (b[1] * x[0]) + (b[2] * x[3]) + (b[3] * x[2]) -
           (a[1] * y[0]) - (a[2] * y[3]) - (a[3] * y[2]);
    y[1] = y[1] / a[0];
  ...
  case 2:
    iter = 3;
    x[2] = sample;
    y[2] = (b[0] * x[2]) + (b[1] * x[1]) + (b[2] * x[0]) + (b[3] * x[3]) -
           (a[1] * y[1]) - (a[2] * y[0]) - (a[3] * y[3]);
    y[2] = y[2] / a[0];
